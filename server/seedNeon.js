...
async function seed() {
  ...
  console.log('Table created. Inserting 100 questions...');
  ...
  for (const item of rawQuestions) {
    ...
    const starter = JSON.stringify(getStarterCode(category));
    const langs = JSON.stringify(['Python', 'JavaScript', 'Java', 'C++']);
    
    await pool.query(`
      INSERT INTO questions (id, title, category, difficulty, description, sample_test_cases, hidden_test_cases, starter_code, supported_languages)
    ...
    `, [qId, title, category, 'Beginner', description, sample, hidden, starter, langs]);
  }
